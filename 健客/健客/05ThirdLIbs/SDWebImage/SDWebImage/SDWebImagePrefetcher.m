...
#ifdef SD_VERBOSE
            LFFLog(@"Prefetched %d out of %d", self.finishedCount, self.prefetchURLs.count);
#endif
        }
        else {
            if (self.progressBlock) {
...
#ifdef SD_VERBOSE
            LFFLog(@"Prefetched %d out of %d (Failed)", self.finishedCount, [self.prefetchURLs count]);
#endif
...
#ifdef SD_VERBOSE
    LFFLog(@"Finished prefetching (%d successful, %d skipped, timeElasped %.2f)", total - self.skippedCount, self.skippedCount, CFAbsoluteTimeGetCurrent() - self.startedTime);
#endif
